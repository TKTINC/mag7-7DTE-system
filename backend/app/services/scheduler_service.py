...
class SchedulerService:
    ...
    async def _run_scheduler(self):
        ...
        while self.running:
            try:
                ...
                await self._check_earnings_alerts(now)
    ...
    async def _check_earnings_alerts(self, now: datetime):
        """Check for earnings alerts to send."""
        # Get database session
        db = next(get_db())
        
        try:
            # Get earnings for tomorrow
            from app.models.reporting import FundamentalData
            from app.models.market_data import Instrument
            
            tomorrow = now.date() + timedelta(days=1)
            
            # Get fundamental data with earnings tomorrow
            earnings_tomorrow = db.query(FundamentalData).filter(
                FundamentalData.next_earnings_date == tomorrow
            ).all()
            
            for earnings in earnings_tomorrow:
                # Get instrument
                instrument = db.query(Instrument).filter(Instrument.id == earnings.instrument_id).first()
                ...
                users = db.query(User).join(
                    ...
                    NotificationPreference.earnings_alerts == True
                ).all()
                
                # Send earnings alerts
                for user in users:
                    if user.email:
                        await self.email_service.send_earnings_alert_email(
                            to_email=user.email,
                            symbol=instrument.symbol,
                            earnings_date=earnings.next_earnings_date,
                            earnings_time=earnings.earnings_time,
                            estimated_eps=earnings.estimated_eps,
                            previous_eps=earnings.previous_eps,
                            price_target=earnings.price_target,
                            analyst_rating=earnings.analyst_rating
                        )
